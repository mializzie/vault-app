...
const Icon = ({ name, size = 20, color = "currentColor" }) => {
  const paths = {
    ...
    unlock: "M19 11H5a2 2 0 00-2 2v7a2 2 0 002 2h14a2 2 0 002-2v-7a2 2 0 00-2-2zM8 7a4 4 0 018 0",
...
export default function PasswordVault() {
  const [screen, setScreen] = useState("splash"); // splash | setup | unlock | vault | add | detail | generator | settings | health | export
  ...
  const [clipTimer, setClipTimer] = useState(null);
  ...
  const [categories] = useState(["All", "Email", "Banking", "Social", "Work", "Shopping", "Other"]);
  ...
  useEffect(() => {
    if (screen === "vault" || screen === "add" || screen === "detail" || screen === "generator" || screen === "settings" || screen === "health" || screen === "export") {
  ...
  const handleBiometric = async () => {
    // Simulate biometric: 80% success, 20% fail (for demo)
    await new Promise(r => setTimeout(r, 1200));
    const success = Math.random() > 0.2;
    if (success) {
      // Auto-fill with demo credentials
      await handleUnlock("DemoPassword123!");
    } else {
      const attempts = biometricAttempts + 1;
      setBiometricAttempts(attempts);
      if (attempts >= 3) {
        setShowMasterFallback(true);
        showToast("⚠️ Biometric failed 3x — use master password", "error");
      } else {
  ...
  const encryptEntry = async (entry, key) => {
    ...
    for (const f of fields) {
      enc[f] = entry[f] ? await CryptoEngine.encrypt(entry[f], key) : "";
    }
  ...
  const decryptEntry = async (entry, key) => {
    ...
    const dec = {};
    for (const f of fields) {
      dec[f] = entry[f] ? await CryptoEngine.decrypt(entry[f], key) : "";
    }
  ...
  const saveEntry = async (entry) => {
    try {
      ...
      if (idx >= 0) rawEntries[idx] = encrypted;
      else rawEntries.push(encrypted);
  ...
  const deleteEntry = async (id) => {
    const rawEntries = (MockDB.get("vault_entries") || []).filter(e => e.id !== id);
    MockDB.set("vault_entries", rawEntries);
  ...
  const getHealthData = () => {
    ...
    const weak = entries.filter(e => getStrength(e.password).label === "Weak");
    const moderate = entries.filter(e => getStrength(e.password).label === "Moderate");
    const dupeEntries = entries.filter(e => dupes[e.password] > 1);
    return { weak, moderate, dupeEntries, total: entries.length };
  };
  ...
  if (screen === "unlock") return (
    <UnlockScreen T={T} onUnlock={handleUnlock} onBiometric={handleBiometric}
      biometricAttempts={biometricAttempts} showMasterFallback={showMasterFallback}
      setShowMasterFallback={setShowMasterFallback} />
  );
  ...
  if (screen === "generator") return (
    <GeneratorScreen T={T} onBack={() => setScreen("vault")} onUse={(pw) => { copyToClipboard(pw, "Generated password"); setScreen("vault"); }} />
  );

  if (screen === "health") return (
    <HealthScreen T={T} data={getHealthData()} onBack={() => setScreen("vault")}
      onSelect={(e) => { setSelectedEntry(e); setScreen("detail"); }} />
  );

  if (screen === "export") return (
    <ExportScreen T={T} onExport={exportVault} onBack={() => setScreen("settings")} />
  );

  if (screen === "settings") return (
    <SettingsScreen T={T} autoLockSetting={autoLockSetting} setAutoLockSetting={setAutoLockSetting}
      theme={theme} setTheme={setTheme} onBack={() => setScreen("vault")}
      onLock={lockVault} onExport={() => setScreen("export")} entryCount={entries.length} />
  );
    ...
    <div style={{ ...styles.container, background: T.bg, color: T.text, fontFamily: "'Courier New', monospace" }}>
      ...
      <div style={{ ...styles.header, background: T.surface, borderBottom: `1px solid ${T.border}` }}>
        ...
        <div style={styles.headerRight}>
          <button onClick={() => setScreen("health")} style={{ ...styles.iconBtn, color: T.textMuted }} title="Health Check">
            <Icon name="health" size={18} />
          </button>
          <button onClick={() => setScreen("generator")} style={{ ...styles.iconBtn, color: T.textMuted }} title="Generator">
            <Icon name="zap" size={18} />
          </button>
          <button onClick={() => setScreen("settings")} style={{ ...styles.iconBtn, color: T.textMuted }} title="Settings">
            <Icon name="settings" size={18} />
          </button>
          <button onClick={lockVault} style={{ ...styles.iconBtn, color: T.danger }} title="Lock">
      ...
      <div style={{ padding: "12px 16px", background: T.surface, borderBottom: `1px solid ${T.border}` }}>
        <div style={{ ...styles.searchBox, background: T.inputBg, border: `1px solid ${T.border}` }}>
          ...
          <input value={searchQ} onChange={e => setSearchQ(e.target.value)}
            placeholder="Search credentials…" style={{ ...styles.searchInput, color: T.text, background: "transparent" }} />
        </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "8px 0" }}>
        {filteredEntries.length === 0 ? (
          <div style={{ textAlign: "center", padding: "60px 24px", color: T.textMuted }}>
            ...
            <div style={{ fontSize: 12, color: T.textMuted }}>{searchQ ? "Try a different search" : "Tap + to add your first credential"}</div>
          </div>
        ) : filteredEntries.map(entry => (
          <EntryCard key={entry.id} entry={entry} T={T}
            onClick={() => { setSelectedEntry(entry); setIsEditing(false); setScreen("detail"); }} />
        ))}
      </div>
...
function EntryCard({ entry, T, onClick }) {
  ...
  const colors = ["#FF6B6B", "#FFD93D", "#6BCB77", "#4D96FF", "#FF6B6B", "#C77DFF", "#FF9A3C"];
  const avatarColor = colors[entry.serviceName?.charCodeAt(0) % colors.length] || T.accent;
  return (
  ...
  return (
    <div style={{ ...styles.fullCenter, background: "#0A0A0F", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ marginTop: 20, display: "flex", gap: 6 }}>
        {[0, 1, 2].map(i => (
          <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: "#00FF94", opacity: 0.4, animation: `bounce 1s ${i * 0.2}s infinite` }} />
        ))}
      </div>
...
// ─── Setup Screen ──────────────────────────────────────────────────────────────
function SetupScreen({ T, onSetup }) {
  ...
  return (
    <div style={{ ...styles.fullCenter, background: T.bg, flexDirection: "column", padding: 32, gap: 0 }}>
        ...
        <div style={{ background: T.surface, borderRadius: 16, padding: 24, border: `1px solid ${T.border}`, display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ background: T.inputBg, borderRadius: 10, padding: 12, border: `1px solid ${T.border}` }}>
            <div style={{ fontSize: 11, color: T.textMuted, lineHeight: 1.6 }}>
              🔐 Your password is encrypted with <strong style={{ color: T.accent }}>AES-256-GCM</strong> using a key derived via <strong style={{ color: T.accent }}>PBKDF2</strong> (310,000 iterations). It never leaves your device.
            </div>
          </div>

          <button onClick={handleSubmit} disabled={pw.length < 8 || pw !== confirm}
            style={{ ...styles.primaryBtn, background: pw.length >= 8 && pw === confirm ? T.accent : T.border, color: pw.length >= 8 && pw === confirm ? "#000" : T.textMuted, cursor: pw.length >= 8 && pw === confirm ? "pointer" : "not-allowed" }}>
            Create Vault →
          </button>
        </div>
      </div>
...
// ─── Unlock Screen ─────────────────────────────────────────────────────────────
function UnlockScreen({ T, onUnlock, onBiometric, biometricAttempts, showMasterFallback, setShowMasterFallback }) {
  const [pw, setPw] = useState("");
  const [show, setShow] = useState(false);
  ...
  return (
    <div style={{ ...styles.fullCenter, background: T.bg, flexDirection: "column", padding: 32 }}>
      <div style={{ width: "100%", maxWidth: 360, textAlign: "center" }}>
        ...
        <div style={{ fontSize: 13, color: T.textMuted, marginTop: 6, marginBottom: 32 }}>Authenticate to access your credentials</div>

        {!showMasterFallback ? (
          <>
            {/* Biometric button */}
            <button onClick={triggerBio} disabled={bioPending}
              style={{ width: "100%", padding: "16px", borderRadius: 14, background: T.surface, border: `2px solid ${T.accent}44`, color: T.text, cursor: bioPending ? "wait" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, fontSize: 15, fontWeight: 600, marginBottom: 16 }}>
              <Icon name="fingerprint" size={24} color={T.accent} />
              {bioPending ? "Authenticating…" : "Use Biometric (Face ID / Touch ID)"}
            </button>

            {biometricAttempts > 0 && (
              <div style={{ fontSize: 12, color: T.danger, marginBottom: 12 }}>
                ⚠️ {biometricAttempts}/3 attempts failed
              </div>
            )}

            <button onClick={() => setShowMasterFallback(true)}
              style={{ background: "none", border: "none", color: T.textMuted, fontSize: 13, cursor: "pointer", textDecoration: "underline" }}>
              Use Master Password instead
            </button>
          </>
        ) : (
          <div style={{ background: T.surface, borderRadius: 16, padding: 20, border: `1px solid ${T.border}`, display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <button onClick={() => onUnlock(pw)}
              style={{ ...styles.primaryBtn, background: T.accent, color: "#000" }}>
              Unlock →
            </button>
            <button onClick={() => setShowMasterFallback(false)}
        ...
        <div style={{ marginTop: 24, fontSize: 11, color: T.textMuted }}>
          Demo hint: password is <span style={{ color: T.accent, fontFamily: "monospace" }}>DemoPassword123!</span>
        </div>
      </div>
    </div>
  );
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        {showGen && (
          <div style={{ background: T.surface, borderRadius: 12, padding: 14, border: `1px solid ${T.border}` }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: T.text, marginBottom: 10 }}>⚡ Password Generator</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <input type="range" min="8" max="32" value={genOpts.length}
                  onChange={e => setGenOpts(o => ({ ...o, length: +e.target.value }))} style={{ flex: 1 }} />
              </div>
...
function DetailScreen({ T, entry, onEdit, onDelete, onCopy, clipCount, onBack }) {
  ...
  const strength = getStrength(entry?.password);
  const toggleReveal = (field) => setRevealedFields(f => ({ ...f, [field]: !f[field] }));
  ...
  return (
    <div style={{ ...styles.container, background: T.bg }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        {/* Fields */}
        {[
          { key: "username", label: "Username", icon: "key", sensitive: false },
          { key: "email", label: "Email", icon: "key", sensitive: false },
          { key: "password", label: "Password", icon: "lock", sensitive: true },
          { key: "notes", label: "Notes", icon: "edit", sensitive: false },
        ].filter(f => entry[f.key]).map(({ key, label, icon, sensitive }) => (
          <div key={key} style={{ background: T.surface, borderRadius: 12, padding: 14, border: `1px solid ${T.border}` }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ flex: 1, fontSize: 14, color: T.text, wordBreak: "break-all", fontFamily: sensitive ? "monospace" : "inherit" }}>
                {sensitive && !revealedFields[key] ? "••••••••••••" : entry[key]}
              </span>
              <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                {sensitive && (
                  <button onClick={() => toggleReveal(key)} style={{ ...styles.iconBtn, color: T.textMuted }}>
                    <Icon name={revealedFields[key] ? "eyeOff" : "eye"} size={16} />
                  </button>
                )}
    ...
    <div style={{ ...styles.container, background: T.bg }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={{ background: T.surface, borderRadius: 16, padding: 20, border: `2px solid ${T.accent}44` }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 12 }}>
            <div style={{ flex: 1, height: 4, background: T.border, borderRadius: 2, overflow: "hidden" }}>
              <div style={{ width: `${(strength.score / 6) * 100}%`, height: "100%", background: strength.color, borderRadius: 2, transition: "all .3s" }} />
            </div>
        ...
        <div style={{ background: T.surface, borderRadius: 16, padding: 18, border: `1px solid ${T.border}`, display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span style={{ fontSize: 10, color: T.textMuted }}>8</span>
              <span style={{ fontSize: 10, color: T.textMuted }}>32</span>
            </div>
          </div>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button onClick={() => onUse(pw)} style={{ flex: 1, ...styles.primaryBtn, background: T.accent, color: "#000" }}>
            Copy & Use
          </button>
        </div>
      </div>
    ...
    <div style={{ ...styles.container, background: T.bg }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
          {[["Total", data.total, T.accent], ["Weak", data.weak.length, "#FF4757"], ["Duplicates", data.dupeEntries.length, "#FFA502"]].map(([label, val, color]) => (
        ...
        {/* Issues */}
        {data.weak.length > 0 && (
          <IssueSection title="⚠️ Weak Passwords" entries={data.weak} T={T} onSelect={onSelect} color="#FF4757" />
        )}
        {data.dupeEntries.length > 0 && (
          <IssueSection title="🔁 Duplicate Passwords" entries={data.dupeEntries} T={T} onSelect={onSelect} color="#FFA502" />
        )}
        {data.weak.length === 0 && data.dupeEntries.length === 0 && (
          <div style={{ textAlign: "center", padding: "30px", color: "#2ED573" }}>
    ...
    <div style={{ background: T.surface, borderRadius: 14, border: `1px solid ${color}44`, overflow: "hidden" }}>
      ...
      {entries.map(e => (
        <button key={e.id} onClick={() => onSelect(e)} style={{ width: "100%", padding: "12px 16px", background: "none", border: "none", borderBottom: `1px solid ${T.border}`, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <span style={{ fontSize: 11, color: color }}>Fix →</span>
        </button>
      ))}
    </div>
  );
  ...
  return (
    <div style={{ ...styles.container, background: T.bg }}>
      ...
      <div style={{ flex: 1, padding: 16, display: "flex", flexDirection: "column", gap: 14 }}>
        <div style={{ background: T.surface, borderRadius: 14, padding: 16, border: `1px solid #FFA50244` }}>
          <div style={{ fontSize: 13, color: "#FFA502", fontWeight: 600, marginBottom: 6 }}>⚠️ Security Notice</div>
          <div style={{ fontSize: 12, color: T.textMuted, lineHeight: 1.6 }}>
            Your backup will be encrypted with AES-256-GCM using the password you set below. Store this backup file and password in a safe location.
          </div>
        </div>
        ...
        <div style={{ background: T.surface, borderRadius: 14, padding: 16, border: `1px solid ${T.border}`, display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <button onClick={() => pw.length >= 6 ? onExport(pw) : alert("Please enter a backup password (min 6 chars)")}
            style={{ ...styles.primaryBtn, background: T.accent, color: "#000" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
              <Icon name="download" size={16} /> Export Encrypted Backup
            </div>
          </button>
        </div>

        <div style={{ background: T.surface, borderRadius: 12, padding: 14, border: `1px solid ${T.border}` }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: T.text, marginBottom: 8 }}>Encryption Flow</div>
          <div style={{ fontSize: 11, color: T.textMuted, lineHeight: 1.8, fontFamily: "monospace" }}>
            {["1. New random salt generated", "2. Export key derived via PBKDF2", "3. Data encrypted with AES-256-GCM", "4. Salt + ciphertext saved to JSON"].map((line, i) => (
              <div key={i}>→ {line}</div>
            ))}
          </div>
        </div>
...
// ─── Settings Screen ───────────────────────────────────────────────────────────
function SettingsScreen({ T, autoLockSetting, setAutoLockSetting, theme, setTheme, onBack, onLock, onExport, entryCount }) {
  return (
    <div style={{ ...styles.container, background: T.bg }}>
      <div style={{ ...styles.header, background: T.surface, borderBottom: `1px solid ${T.border}` }}>
        <button onClick={onBack} style={{ ...styles.iconBtn, color: T.textMuted }}><Icon name="x" size={20} /></button>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={{ background: T.surface, borderRadius: 14, border: `1px solid ${T.border}` }}>
          ...
          {[["immediate", "Immediately"], ["1min", "After 1 minute"], ["5min", "After 5 minutes"], ["never", "Never"]].map(([val, label], i, arr) => (
        ...
        <div style={{ padding: "12px 0", textAlign: "center" }}>
          <div style={{ fontSize: 11, color: T.textMuted }}>Password Vault v1.0</div>
          <div style={{ fontSize: 10, color: T.textMuted, marginTop: 4 }}>WebCrypto AES-256-GCM · PBKDF2 · Local Storage Only</div>
        </div>
      </div>
    </div>
  );
